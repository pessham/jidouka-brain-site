...
(ちなみに上記2つとも教材購入者さんには[第4章「コードのダウンロード」](#第4章-コードのダウンロード)でZIPファイルとしてお渡ししていますので、ぜひ活用してみてください！)
...
**.command でスクリプトを直接動かしたい**場合は.command、
...
調べたら、入れ方の違いでした。
公式インストーラー版の最新Pythonは、出たばかりで `venv` がうまく動かないことがある。

ここで、痛いほど分かったことがあります。

> **「自分の環境で動いた」は、「全員の環境で動く」ではない。**

これ、`browser-automation-principles` の「**操作したか、ではなく結果が変わったか**」の配布版なんですよね。

自分のマシンで動いたのは「操作が通った」だけ。
渡した相手のマシンで動いて、初めて「目的が達成された」になる。

スクリプトを直しても、僕は「次は絶対動きます」と言えませんでした。
だって、相手の環境では試せていないんですから。

### 道具選びを、間違えていた

直しながら、もっと根っこの判断ミスに気づきました。

**配るものを、`.command` で作ったこと自体**が間違いでした。

`.command` は、相手にPythonを入れさせ、ターミナルを触らせ、環境構築をさせる。
非エンジニアにとって、入口のハードルが高すぎます。

配るなら、**Chrome拡張**のほうがよかった。
Chromeに追加するだけ。Python不要。ターミナル不要。

「でも拡張機能って作るの大変じゃないの？」と、昔の僕なら思っていました。

ところが、**Claude Code等 に作らせるなら、構築コストはほぼ同じ**なんです。

つまり、判断基準は「作りやすさ」ではなく「**相手にとっての使いやすさ**」だけでよかった。
...
「**全ユーザーが、エラーなく使えるか**」を、軸を立てて網羅的にチェックしたんです。
...
Chrome拡張は、`key` という設定をしないと、**入れたフォルダの場所からIDが自動生成**されます。

つまり、僕のMacと、参加者のパソコンでは、**拡張機能のIDが変わる**。

Googleの認証はそのIDと紐づいているので、IDが違えば**他の人は全員ログインで弾かれる**。

…またしても、「**自分のマシンでしか動かない**」状態だったんです。

`key` を設定してIDを固定し、別アカウントで実際に通ることを確認して、ようやく配れる状態になりました。

LP も作り直して、6月のセミナー参加者への特典として配布しています。
[https://brain-scraper-lp.vercel.app/](https://brain-scraper-lp.vercel.app/)

### この実例で学んだこと

人に渡すものは、**自分の環境で動いた時点では、半分も終わっていません**。
...
この章で紹介した実例のうち、
4つのコードを ZIP ファイルで配布します。

| ファイル名 | 実例 |
|---|---|
| `02_podcast-upload-template.zip` | 実例②　音声配信の毎日アップロード |
| `03_note-auto-template.zip` | 実例③　文字起こし → note 自動投稿 |
| `04_substack-digest-template.zip` | 実例④　Substack 登録者バックアップ |
| `05_invoice-saver-template.zip` | 実例⑤　Gmail 請求書PDF 自動収集 |
...
このコードは「僕の環境で動いているもの」をそのまま渡しています。
アカウント情報、フォルダのパス、使っている GPT の URL など、
自分の環境に合わせて書き換えが必要な箇所があります。

スクリプトの冒頭に `★` マークでコメントを入れてあるので、
そこを探して書き換えるところから始めてください。

書き換えが終わったら、README.md（または実行方法.md）の手順通りに進めると動きます。

わからなくなったら、Claude Code に「このスクリプトを動かすには何を設定すればいい？」と聞いてください。
コードを丸ごと渡せば、必要な設定を教えてくれます。
