...
この教材では、「クリプトニンジャ」より、
AIや自動化に慣れていない「餡音」と、
それを教える「ハヤテ師匠」の会話を交えながら進めます。

キャラクターを知らなくても読めるので、
まずは「自動化初心者の読者代表」くらいに見てください。

![](https://jidouka.pessham.com/images/anne_intro.png)

![](https://jidouka.pessham.com/images/hayate_intro.png)
...
そこからNFTバブルの勢いに乗って、
**4,000万円ほど**のお金が動くプロジェクトを経験、その流れで、大企業の安定の道を捨てて、
起業しちゃいました。
...
>
> 本教材を購入後、レビューいただいた方に、
> こちらの音声配信自動アップロードのファイルをお渡しします！
>
> 後日になりますが、
> SpotifyとStand FM、
> SpotifyとVoicyバージョンのファイルも作る予定です。
>
> ご自身の環境にアップデートの上、ご利用いただけます。
...
## 追加特典　ブラウザ自動化スキル「jidouka」（2026年7月追加・8月にWindows手順を追加）

「スキル」とは、AIに私のやり方を
そのまま覚えさせておくファイル一式です。
Claude Codeに入れておくと、自動化を頼んだとき、私が実際に使っている手順に沿って動くようになります。

一度手を動かしてみた方に向けた特典です。
教材を読んで実際に作ってみた方、途中で詰まって止まっている方、昔作ったものが動かなくなった方を想定しています。
まだ何も作っていない方も使えますが、先に特典①②と第0章を進めたほうが効きます。

正直に書いておきます。いまのClaude Codeは、
スキルを入れなくてもかなりちゃんと自動化を書きます。
ゼロから1本作るだけなら、差はそれほど大きくありません。
効いてくるのは、止まっているものを動かし直すときです。
半年前のフォルダをAIが読んで、いつどこで止まったかを示すところ、
使い続けるうちにたまる運用の劣化を直すところに、
実際に踏んだ人でないと書けない部分を入れています。

前提は、Claude Code（有料プラン）とPython 3です。
v2.1.0から、MacとWindowsの両方に対応しました。
ただし、Windowsは実機での動作確認がまだ済んでいません。
ファイルと手順は入れてありますが、実機で確かめられているのはMacだけです。
設置手順はClaude Code専用です。ほかのAIツール向けの手順は、今回のバージョンには入っていません。

### ダウンロードのやり方

1. 下のリンク（またはボタン）を押すと、`jidouka.zip` のダウンロードが始まります。画面が切り替わらなくても、失敗ではありません。
2. ファイルは「ダウンロード」フォルダに入ります。
3. ここから先は、zipのままClaude Codeに頼むのがいちばん確実です。解凍もフォルダ移動も要りません。Claude Codeを開いて、こう言ってください。
   > 「ダウンロードフォルダにある jidouka.zip（または解凍済みの jidouka フォルダ）を、スキルとして `~/.claude/skills/` に設置してください。設置できたら、jidouka スキルが読めるか確認してください」
4. 解凍から設置、読み込み確認まで、Claude Codeがやってくれます。終わったら「自動化したい」と話しかければ、もう使えます。
5. 自分の手で入れたい方向けの手順は、zipを解凍して出てくる「jidouka」フォルダの中の「はじめにお読みください.md」に書いてあります（Windowsは右クリック→「すべて展開」で解凍してから開いてください）。手作業でも10分ほどで終わります。
6. うまくダウンロードできない、設置がうまくいかないといったときは、特典③のLINEオープンチャットまでご連絡ください。

👉 **[スキル「jidouka」をダウンロード](https://jidouka.pessham.com/jidouka.zip)**

更新は、特典③のLINEオープンチャットで告知します。
...
それでも動かなかったら、ご自身の Claude Code等 に
「このファイルが動かないので、動くように修正してください」
とそのままお願いしてください。
エラー画面やフォルダごと渡すと、直してもらいやすくなります。
...
まず[**特典2：ブラウザ自動化スターターキット**](https://browser-automation-starter-kit.vercel.app/)を使って、
今の業務を棚卸しして整理しましょう。
...
`.command` ファイルは自分用なら速いです。
ただ、人に配ると、相手の Python、
OS、フォルダ構成、権限設定にかなり左右されます。

配布するなら、**Chrome拡張の方が向いている場面が多い**です。
Chromeに入れてもらうだけで済むので、
相手の環境差を減らせます。

`.command` ファイルで動かないときは、
そのまま Claude Code等 に投げて、
**自分の環境で動く形に作り替えてもらう**
のが現実的です。

- YES → Chrome拡張（自作）
- NO → .command / Playwright / Claude in Chrome で十分
...
**まず、[特典2：ブラウザ自動化スターターキット](https://browser-automation-starter-kit.vercel.app/)で今の業務を棚卸ししてください。**
...
棚卸しが終わったら、
[**特典1：業務自動化ネタ出しアシスタント（GPTs）**](https://chatgpt.com/g/g-6a0ece0ca25c819190141566263d44ff-ye-wu-zi-dong-hua-netachu-siasisutanto)
で候補を具体化してください。
...
人に渡す場合、`.command` ファイルは相手の環境に左右されやすいです。
Python のバージョン、OS、フォルダの場所、権限設定で止まることがあります。
そのときは、止まったファイルやエラー画面を
そのまま Claude Code等 に渡して、
**自分の環境で動く形に作り替えてもらう**
のが現実的です。
...
まず[**特典2：ブラウザ自動化スターターキット**](https://browser-automation-starter-kit.vercel.app/)で、今やっている業務を棚卸ししてください。
...
**ブラウザは画面に表示した状態で動かしてください**。裏でこっそりではなく。
...
Phase 1 は、
**AIが録画を元に作業の流れを説明できるようになったら完了**です。

まだコードは完成していなくて大丈夫です。
この段階で大事なのは、

**「AIと自分が、同じ作業を見ている状態」を作ることです。**
ここがズレたままコードを書かせると、
あとで大きくズレます。
...
**コードに直接パスワードを書いてしまうと、うっかり GitHub に公開してしまったり、スクショを撮ったときに映り込んだりします。**
...
`headless=False`（つまり、ブラウザを画面に表示して動かすモード）にしておかないと、どこで止まったかが見えません。

ログイン画面で止まったのか、
ファイル選択で止まったのか、
**裏側で動いているとまったく分からないまま終わります**。
...
> **ここで詰まるのが普通です。**
> Phase 3 は、完成させる章というより
「どこで止まるかを見つける章」です。
> 詳しい直し方は [第3章](#第3章) で扱います。
ここでは、最初に集める材料だけ押さえます。
...
ここから先の詳しい直し方は、
[第3章の「報告テンプレート」](#第3章-報告テンプレート（コピペok）) と 
[第3章の「第2階層の抜け方」](#第3章-第2階層の抜け方) を使ってください。
...
CDP(Chrome DevTools Protocol)方式を
一言で説明すると、
「**Playwrightがあなたの使っているChromeに後からつなぐ方法**」です。

ふつうの Playwright は、
新しいブラウザをゼロから自動で起動します。

このとき、
ブラウザに「自動化ツールが起動した」という情報が残ります。
これを見てサービス側が「人間じゃない」
と判断することがあります。

CDP方式は違います。
**あなたがいつも使っているChromeを普通に手で開き、手でログインする。** 

その後、
Playwright はそのChromeに「後からつなぐ」
だけです。

ブラウザは最初から人間が起動しているので、
「自動化された新しいブラウザ」
とは区別されます。
...
「**人間がエンジンをかけてシートベルトをして、あとはロボットにハンドルを渡す**」
ような形です。
...
実際にこの方法で
 **Substack・ChatGPT・note** の
 ログイン認証を突破できました。

Cloudflare 系（X、LinkedIn など）
や Proton といったセキュリティが極めて強いサイトには効きませんが、
**「ふつうの方法で詰まったらCDP」と覚えておくだけで、選択肢が大きく広がります**。
...
このサインが出ると、厳しいサービスは
「**メールを送ったフリをして実際には送らない**」といったような挙動をします。
エラーも出ないので、
**気づかないままハマり続けます**。
...
ログインに必要なのは、
実は
**一度このセッションを通りましたという通行手形** （Cookie）だけなんです。
...
- [第3章「抜け方3：人間がやる『切り分け』」](#第3章-抜け方3：人間がやる「切り分け」)
...
たとえば、
**サービスAへのアップロードが終わったあとにサービスBで止まったとします**。
最初から全部やり直すと、サービスAに二重投稿になるかもしれません。
...
あらかじめ定義しておく仕組みです。
 ...
 スキルは
 「**Claude Codeへの指示をワンコマンドで呼び出す入口**」です。
...
次回から、
**Claude Code 等で /〇〇 などと打つだけ**で、
Claude Codeがプロジェクトの文脈を把握した状態で作業を始めてくれます。
...
**.command でスクリプトを直接動かしたい**場合は.command、または.bat
定期実行させたいならlaunchd、またはタスクスケジューラ
**Claude Code 等から動かしたい**場合はスキル、
という使い分けになります。
...
Playwright は「**ブラウザを動かす**」ChatGPT Tasks は「**AIに定期的に考えてもらう**」
目的が違うので、
組み合わせて使うのが自然です。
...
ここまでで、Playwrightで自動化を作る型は渡しました。
...
**実際にどんな自動化を作ったか**は、
第4章（事例集）で紹介します。
音声配信アップロード、業務委託のページ作成、予約メール転記など、
**業務ごとの詰まりとその抜け方**を
「録る・作る・直す・育てる」
に沿って並べています。
...
Grok は、SNSや最近のサービス仕様で別の見方が出ることがあります。
...
この発想は、ブラウザ自動化に限らず、
**別のAIにバトンを渡すあらゆる場面で使えます**。覚えておくと、本当に得します。
...
> **この章で紹介するコードは、後半でダウンロードできます。**
>
> 実例②〜⑤のスクリプトを ZIP ファイルにまとめています。
> 読み終わったら、自分の環境に合わせてカスタマイズして使ってみてください。
...
15分が、タイトル入力の10秒になりました。

---

### Phase 1：2サービスの作業を録画した

音声配信サービス A・B への投稿作業を、
1本の動画で録画しました。

「**2サービスを順番に操作して投稿する作業**」
と Claude Code に説明しました。
...
**Claude Code** に引き継ぎ書を書いてもらって、
**Codex** に投げました。

返ってきた答えは「**launchd** から起動した bashには Downloads フォルダへのアクセス権がない。macOS の TCC（つまり、アプリごとにフォルダのアクセス権を管理する仕組み）が原因」ということでした。
...
解決策は **launchd** をやめて、
Homebrew のfswatch（ファイルの変化を監視して処理を動かすツール）を使うということでした。

エンジニアじゃないからちんぷんかんぷんですが、**一つのAIでは気づけなかったことが、他のAIに聞くことで気づくことができた**、ということだけ押さえておいてください。
...
「--skip-service-a というオプションを追加して、A をスキップして B から始められる形にして」
と頼みました。
...
毎朝 iPhone で録音した音声を、
note と Substackの記事にする
までの手順はこうでした。

```
1. iPhone から AirDrop で音声ファイルを Mac に送る
2. Google AI Studio で文字起こし（ここで数分待つ）
...
Google AI Studio は精度が高い文字起こしツールです。
...
たどり着いたのが [**CDP**、つまり Chrome の開発者ツールに外から接続してブラウザを操作する方式-詳しくは第2章参照）](#第2章-ログインで詰まったら：cdp方式を試す)です。

すでにログイン済みの Braveブラウザのクッキー（ログインした時の情報）をそのまま読み取って使います。
...
Proton（E2E 暗号化＝外からの自動操作に強く抵抗するセキュリティ設計のサービス）
へのファイル自動インポートが、
どうやっても突破できませんでした。
...
最終的な結論は
「**Proton への貼り付けは手動でやる**」でした。

全部自動化できなくても、ここは手動でいい、
という判断です。

「**10分かかっていた作業が7分になった**」なら、
それで十分でした。
...
Gmail関係はPlaywrightのログインで詰まる
それを知っていた私は、MCPを見にいきました。

Claude Code等 では **Gmail MCP**
（AIが Gmail に直接アクセスするための接続口）が使えます。
...
でも、
添付ファイルのダウンロードで止まりました。
Gmail MCP には、添付ファイルを取得する機能がそもそもなかったのです。

### Phase 2：IMAP に切り替えた

Claude Code等 に
「**Gmail MCPでは添付ファイルが取れなかったので、別の方法で取りたい**」と伝えました。
...
技術的には Next.js（Webアプリを作るフレームワーク）と Supabase（ユーザー認証やデータ管理を担うサービス）
の組み合わせです。
...
Chromeのツールバーのアイコンを押して、スプレッドシートの URL を貼り付けて「転記する」を押す。

それだけで上位20件がスプシに書き込まれます。

![](https://jidouka.pessham.com/images/ch04/brain_scraper.png)

Pythonのインストールも、ターミナルもいりません。

…ただ、ここに辿り着くまでが、いちばん勉強になりました。

この実例は「きれいに動いた話」ではなく、「**配ってから何度もコケた話**」です。
そこにいちばん価値があると思うので、そのまま書きます。

---

### 最初は、コードで作った

最初のバージョンは、ダブルクリックで動く `.command`（Mac用）と `.bat`（Windows用）で作りました。

理由は単純で、**そのほうが作るのが速かった**からです。

Brain のスクレイプも、スプシへの書き込みも、Claude Code等 に頼んだらすぐ動きました。
...
セミナーオーナーの方に渡したら、セットアップで止まりました。

エラーは「**仮想環境（venv）の作成に失敗しました**」。

その方のPythonは、公式サイトから入れた最新の **3.14**。

僕のMacに入っていたのは、Homebrewで入れた **3.14**。

同じ3.14なのに、片方は動いて、片方は動かない。

調べたら、入れ方の違いでした。
公式インストーラー版の最新Pythonは、
出たばかりで `venv` がうまく動かないことがあるとのこと。

ここで、痛いほど分かったことがあります。

> **「自分の環境で動いた」は、「他の人全員の環境で動く」ではない。**

これ、`browser-automation-principles` の「**操作したか、ではなく結果が変わったか**」
と根本は同じなんですよね。

自分のマシンで動いたのは「**操作が通った**」だけ。
渡した相手のマシンで動いて、
初めて「**目的が達成された**」になる。

スクリプトを直しても、
僕は「**次は絶対動きます**」と言えませんでした。
だって、相手の環境では試せていないんですから。

### 道具選びを、間違えていた

エンジニアの方に相談したら、一発でもっと根っこの判断ミスに気づきました。

**配るものを、`.command` で作ったこと自体**が間違いでした。

`.command` は、相手にPythonを入れさせ、ターミナルを触らせ、環境構築をさせる。
相手の環境によって左右される上に、
非エンジニアにとって、入口のハードルが高いんです。

配るなら、**Chrome拡張**のほうがよかった。
Chrome拡張は環境による差が小さいのだそうです。
さらにPythonもターミナルも不要で、手順はChromeに追加するだけです。

「でも拡張機能って作るの大変じゃないの？」と、昔の僕なら思っていました。

ところが、
**Claude Code等 に作らせるなら、構築コストはほぼ同じ**なんです。

つまり、判断基準は「作りやすさ」ではなく
「**相手にとっての使いやすさ**」だけでいいんです。

そこで、Chrome拡張に作り直しました。

### 配る前に、もれなくダブりなく洗った

一度コケているので、今度は配る前に立ち止まりました。

「**全ユーザーが、エラーなく使えるか**」を、
軸を立てて網羅的にチェックしました。

- 認証（個人Gmail・会社のWorkspace両方で通るか）
- ページ取得（ログイン不要か・0件のとき落ちないか）
- 書き込み（URLの解析・権限）
- インストール（別のパソコンでも入るか）

そこで、配る寸前に**致命的なバグ**が1個見つかりました。

Chrome拡張は、`key` という設定をしないと、
**入れたフォルダの場所からIDが自動生成**されます。

つまり、僕のMacと、参加者のパソコンでは、
**拡張機能のIDが変わってしまいます**。

Googleの認証はそのIDと紐づいているので、
IDが違えば**他の人は全員ログインで弾かれてしまいます**。

…またしても、「**自分のマシンでしか動かない**」状態だったんです。

配る前に気づけてよかった...
`key` を設定してIDを固定し、ようやく配れる状態になりました。

LP も作り直して、教材購入者及び6月のセミナー参加者への特典として配布しています。
...
`browser-automation-principles` の鉄則は、配布でもそのまま効きます。

> 操作が通った(＝自分のマシンで動いた)と、
> 目的が達成された(＝渡した相手のマシンで動いた)は、別物。

だから、配る前にこの2つを習慣にしてください。

1. **道具を、相手の目線で選ぶ。** 配布前提なら、Python不要のChrome拡張が第一候補。AIに作らせるなら、もう「拡張は作るのが大変」という時代ではありません。
2. **配る前に、自分以外の環境でも動くかを、もれなくダブりなく洗う。** 「自分のマシンで動いた」は、出発点であってゴールではない。

僕は、これを2回コケてから学びました。

あなたは、配る前にこのページを思い出してもらえたら、それでいいです。
...
![](https://jidouka.pessham.com/images/shusho/arigatou.png)
